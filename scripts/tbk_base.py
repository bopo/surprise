...
from restful.models.goods import Goods, TBKCategory, Collect
...
for c in channelss:
    c = c.split(' ')
    channels.append({'name': c[0], 'slug': c[1], 'id': k})
    k += 1
...
def navs(channels):
    ...
    for channel in channels:
        req = requests.get(BASE_URL.replace('{channel}', channel['slug']))
        try:
            rep = json.loads(req.content)
            parent, status = TBKCategory.objects.get_or_create(id=channel['id'], name=channel['name'],
        ...
        for x in rep['data']['navigator']:
            ...
            if x.get('subIds'):
                for sub in x.get('subIds'):
                    ...
                    tt, ss = TBKCategory.objects.get_or_create(name=sub.get('name'), id=sub.get('id'))
                ...
                if x.get('id') < 20:
                    continue
                t, s = TBKCategory.objects.get_or_create(name=x.get('name'), id=x.get('id'))
...
def run():
    objects = Collect.objects.filter(open_iid__isnull=True)

...
from restful.models.goods import Preselection, PreselectionCategory
from restful.taobao import import2db
...
def categories():
    ...
    open('nayibang.json', 'w').write(req.content)
...
def subcategories(sub, cat):
    ...
    for item in res.get('data'):
        ...
        try:
            print data['subcategory_id'], data['source']
            pc = PreselectionCategory.objects.filter(subcategory_id=data['subcategory_id']).filter(
                source=data['source']).get()
            data['category_id'] = pc.category_id
            print 'pc.category_id', pc.category_id
        except PreselectionCategory.DoesNotExist:
            raw_input('PreselectionCategory.DoesNotExist')
            pass

        # item[str(data['num_iid'])] = data
        # print item.keys()

        # print data['title'], data['num_iid']

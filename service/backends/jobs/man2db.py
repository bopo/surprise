...
r = requests.get('http://127.0.0.1:8000/api/v1.0/category/1/children/')
CHILDREN = r.json()
# open('cat.json', 'w').write(r.content)
...
def categories():
    ...
    open('nyb.json', 'w').write(req.content)
    ...
    for cat in req.json().get('data'):
        for child in CHILDREN:
            ...
            if cat.get('name') == child.get('name'):
                cat['child'] = child.get('child')
                break

        if not cat.get('child'):
            print cat.get('name'), 'not child'
            continue

        for sub in cat.get('categories'):
            for sc in cat['child']:
                if sub.get('name') == sc.get('name'):
                    sub['child'] = sc
                    break
...
def subcategories(sub, cat):
    ...
    if not sub.get('child'):
        print sub.get('name'), 'sub not child'
        #
